...
from marjapussi.utils import Card  # Add this import statement
...
class MarjaPussi():
    ...
    def __init__(self, player_names, override_rules={}, log=True, fancy=True, language=1) -> None:
        ...
        deck = [Card(c, v) for c in utils.COLORS for v in utils.VALUES]  # Use the Card class here
        shuffle(deck)
    ...
    def legal_pass(self):
        ...
        for card in self.playing_player.partner.cards:
            if str(card) in self.passed_cards["forth"]:
                continue
    ...
    def legal_passing_back(self):
        ...
        for card in self.playing_player.cards:
            if str(card) in self.passed_cards["back"]:
                continue
    ...
    def legal_trck(self):
        return [f"{self.player_at_turn.number},TRCK,{str(c)}" for c in
    ...
    def legal_ques(self):
        ...
        if lvl == 0:
            quests += [f"{self.player_at_turn.number},QUES,my{col}" for col in utils.COLORS
                       if (utils.contains_pair([str(card) for card in self.player_at_turn.cards],
                                               col) and col not in self.all_sup)]
        if lvl <= 1:
    ...
    def legal_answer(self):
        ...
        if quest == "you":
            answ = [f"{self.player_at_turn.number},ANSW,my{col}" for col in utils.COLORS
                    if (utils.contains_pair([str(card) for card in self.player_at_turn.cards], col) and not col in self.all_sup)]
            if not answ:
                return [f"{self.player_at_turn.number},ANSW,nmy"]
            return answ
        else:
            col = quest[-1]
            return [f"{self.player_at_turn.number},ANSW,{(f'ou{col}') if utils.contains_half([str(card) for card in self.player_at_turn.cards],col) else f'no{col}'}"]
    ...
    def act_answ(self, player, answ):
        ...
        if answ[:2] == "ou":
            ...
            if utils.contains_half([str(card) for card in self.player_at_turn.partner.cards], pot_sup):
    ...
    def state_dict(self):
        return {
            ...
            "current_trick": [str(card) for card in self.tricks[-1]],
    ...
    def end_info(self):
        ...
        return {
            ...
            "tricks": [[str(card) for card in trick] for trick in self.tricks],
