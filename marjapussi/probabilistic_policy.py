...
class ProbabilisticPolicy2(Policy):
    ...
    def _assess_own_hand(self, state: GameState):
        ...
        # next, check for halves and pairs
        halves = [card for card in hand_cards if card.value == Value.Koenig or card.value == Value.Ober]
        pairs = []
        if len(halves) >= 2:
            for color in Color:
                if utils.contains_col_pair(hand_cards):
                    pairs.append(color)
        for pair in pairs:
            if pair in["g", "e"]:
