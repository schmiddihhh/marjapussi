...
class ProbabilisticPolicy2(Policy):
    ...
    def select_card(self, state: GameState, legal_actions: list[Action]) -> Action:
            """
            ...
            for action in legal_actions:
                if isinstance(action.content, Card) in standing_cards:
                    return action
    ...
    def select_card_or_question(self, state: GameState, legal_actions: list[Action]) -> Action:
        ...
        standing_cards = state.standing_cards()
        assert [action for action in legal_actions if isinstance(action.content, Card)]
        print( [action for action in legal_actions if isinstance(action.content, Card)])
        ...
        if state.concepts.get_by_name(f"{state.partner()}_has_3+_halves") and len(state.customs['standalone_halves']) >= 2 or \
           state.concepts.get_by_name(f"{state.partner()}_has_2_halves") and len(state.customs['standalone_halves']) >= 3:
            # we have a pair together
            possible_colors = []
            for half in state.customs['standalone_halves']:
                if half.color not in possible_colors:
                    possible_colors.append(half.color)
            if steps_to_pair == None:
                if len(possible_colors) == 0:
                    # we probably have no pair
                    steps_to_pair = math.inf
                else:
                    steps_to_pair = len(possible_colors) + 1    # +1 because we always ask for a pair first
        ...
        if len(current_trick.cards) == 0:
            # we start the trick
            if state.phase == "TRCK":
                # this must be the first trick, since we start it but cannot ask questions
                # just play a standing card if possible, TODO: check if this would destroy a pair
                return self.select_card(state, legal_actions)
            elif state.phase == "QUES":
                # I just won a trick and can either announce a pair, ask a question or play a card
                if steps_to_pair == None or len(standing_cards) > steps_to_pair:
                    return self.select_card(state, legal_actions)
                else:
                    # we have to announce a pair or we might lose control
                    if state.customs['pairs']:
                        for action in legal_actions:
                            if isinstance(action.content, Talk) and action.content == Talk('My', state.customs['pairs'][0]):
                                state.customs['pairs'].pop(0)
                                return action
                    if (concept := state.concepts.get_by_name(f"{state.partner}_has_big_pair")) or (concept := state.concepts.get_by_name(f"{state.partner}_has_small_pair")):
                        for action in legal_actions:
                            if isinstance(action.content, Talk) and action.content == Talk('Yours', None):
                                return action
                    if state.concepts.get_by_name(f"{state.partner()}_has_3+_halves") and len(state.customs['standalone_halves']) >= 2 or \
                       state.concepts.get_by_name(f"{state.partner()}_has_2_halves") and len(state.customs['standalone_halves']) >= 3:
                        for action in legal_actions:
                            if isinstance(action.content, Talk) and action.content == Talk('Our', possible_colors[0]):
